...
void DomainTransform(cv::Mat _src, cv::Mat& _ct, float _sigma_s, float _sigma_r, int _cum = 1);
...
bool SpatialEdgePreservingFilter_v2(cv::Mat _src, cv::Mat& _dst, int _sigma_s, double _sigma_r, int _num_iterations = 3, DOMAIN_TRANSFORM_MODE _mode = NORMALIZED_CONVOLUTION);
...
extern "C" FILTER2D_DLL bool DomainTransformFilter(cv::Mat _src, cv::Mat& _dst, int _sigma_s, double _sigma_r, int _num_iterations = 3, DOMAIN_TRANSFORM_MODE _mode = VERSION_1);